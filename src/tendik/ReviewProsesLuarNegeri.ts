        ...
        const content = `
            <div class="w-full max-w-5xl mx-auto pb-20 animate-fade-in space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                    ...
                    ${canAct ? `
                        <div class="space-y-4">
                            <button id="pln-revise-btn" type="button" class="w-full py-3.5 bg-[#FACC15] hover:bg-[#EAB308] text-yellow-900 font-bold rounded-xl transition-colors border border-[#EAB308]/50 shadow-sm active:scale-[0.98]">
                                Minta Revisi
                            </button>
                            <button id="pln-reject-btn" type="button" class="w-full py-3.5 bg-[#EF4444] hover:bg-[#DC2626] text-white font-bold rounded-xl transition-colors shadow-sm active:scale-[0.98]">
                                Tolak Pengajuan Surat
                            </button>
                            <button id="pln-approve-btn" type="button" class="w-full py-3.5 bg-[#115E59] hover:bg-[#0d4a46] text-white font-bold rounded-xl transition-colors shadow-sm active:scale-[0.98]">
...
function renderActionModals(app: ProsesLuarNegeriApplication, studentName: string, nim: string): string {
    return `
        ${renderApprovalModal(studentName, nim, valueOrDash(app.keperluan))}
        ${renderTextActionModal({
            id: 'pln-revision-modal',
            ...
            confirmId: 'pln-confirm-revise',
            cancelId: 'pln-cancel-revise',
            textareaId: 'pln-revision-note',
        ...
        ${renderTextActionModal({
            ...
            confirmId: 'pln-confirm-reject',
            cancelId: 'pln-cancel-reject',
            textareaId: 'pln-rejection-reason',
...
function renderApprovalModal(studentName: string, nim: string, purpose: string): string {
    return `
        <div id="pln-approval-modal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 animate-fade-in" role="dialog" aria-modal="true" aria-labelledby="pln-approval-title">
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
                <div class="bg-[#115E59] px-8 py-5 text-white">
                    <h3 id="pln-approval-title" class="text-lg font-bold">Konfirmasi Persetujuan</h3>
                </div>
                <div class="p-8 space-y-6">
                    ...
                    <div class="space-y-2">
                        <label for="pln-nomor-surat" class="text-[11px] font-black text-gray-500 uppercase tracking-widest pl-1">Tambahkan Nomor Surat</label>
                        <input
                            id="pln-nomor-surat"
                            type="text"
                ...
                <div class="px-8 pb-8 flex gap-3">
                    <button id="pln-cancel-approve" type="button" class="flex-1 px-6 py-3.5 border-2 border-gray-100 text-gray-500 font-bold rounded-2xl hover:bg-gray-50 transition-colors text-sm shadow-sm">
                        Batal
                    </button>
                    <button id="pln-confirm-approve" type="button" class="flex-[1.5] px-6 py-3.5 bg-[#115E59] text-white font-bold rounded-2xl hover:bg-[#0d4a46] transition-all shadow-lg active:scale-[0.98] text-sm">
...
function bindActionHandlers(applicationId: number, role: string): void {
    ...
    document.getElementById('pln-confirm-approve')?.addEventListener('click', async () => {
        const nomorSurat = readTextareaOrInput('pln-nomor-surat');
        if (!nomorSurat) {
    ...
    document.getElementById('pln-confirm-revise')?.addEventListener('click', async () => {
        const note = readTextareaOrInput('pln-revision-note');
        if (!note) {
    ...
    document.getElementById('pln-confirm-reject')?.addEventListener('click', async () => {
        const reason = readTextareaOrInput('pln-rejection-reason');
        if (!reason) {
    ...
    document.addEventListener('keydown', function closeProsesLuarNegeriModalsWithEsc(event) {
        if (event.key === 'Escape') {
...
function closeAllProsesLuarNegeriModals(): void {
    ['pln-approval-modal', 'pln-revision-modal', 'pln-rejection-modal'].forEach((id) => {
