...
const API_PREFIX = '/api/tendik/surat-keterangan-aktif';
const GENERATED_LETTER_PREVIEW_ROOT_ID = 'aktif-tendik-generated-letter-preview';
...
export const renderReviewSuratKeteranganAktif = async (applicationId: number, options?: ReviewerNavigationOptions) => {
    ...
    try {
        ...
        // Canonical academic display: read from the relation tree first, fall back to the
        // legacy mahasiswa_profiles.{program_studi,fakultas} text columns only if absent.
        ...
        const content = `
            <div class="w-full max-w-5xl mx-auto pb-20 animate-fade-in space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                    ...
                    ${canAct ? `
                        <div class="space-y-4">
                            <button id="aktif-revise-btn" type="button" class="w-full py-3.5 bg-[#FACC15] hover:bg-[#EAB308] text-yellow-900 font-bold rounded-xl transition-colors border border-[#EAB308]/50 shadow-sm active:scale-[0.98]">
                                Minta Revisi
                            </button>
                            <button id="aktif-reject-btn" type="button" class="w-full py-3.5 bg-[#EF4444] hover:bg-[#DC2626] text-white font-bold rounded-xl transition-colors shadow-sm active:scale-[0.98]">
                                Tolak Pengajuan Surat
                            </button>
                            <button id="aktif-approve-btn" type="button" class="w-full py-3.5 bg-[#115E59] hover:bg-[#0d4a46] text-white font-bold rounded-xl transition-colors shadow-sm active:scale-[0.98]">
                            ...
                            <div class="mt-4 bg-[#FEF9C3]/50 border border-[#FEF08A] rounded-xl p-4 text-xs font-medium text-amber-900 shadow-inner">
                                <p><span class="font-bold">Catatan:</span> Nomor surat wajib diisi manual saat menyetujui pengajuan.</p>
                            </div>
                        </div>
                    ` : `
...
function renderActionModals(app: AktifApplication, studentName: string, nim: string): string {
    return `
        ${renderApprovalModal(studentName, nim, valueOrDash(app.keperluan))}
        ${renderTextActionModal({
            id: 'aktif-revision-modal',
            title: 'Permintaan Revisi',
            headerClass: 'bg-[#FACC15] text-gray-900',
            label: 'Catatan Revisi untuk Mahasiswa',
            placeholder: 'Contoh: Keperluan pengajuan perlu diperjelas sebelum diproses.',
            helper: 'Tuliskan instruksi revisi secara jelas agar mahasiswa mengetahui bagian yang perlu diperbaiki.',
            confirmId: 'aktif-confirm-revise',
            cancelId: 'aktif-cancel-revise',
            textareaId: 'aktif-revision-note',
        ...
        ${renderTextActionModal({
            ...
            confirmId: 'aktif-confirm-reject',
            cancelId: 'aktif-cancel-reject',
            textareaId: 'aktif-rejection-reason',
...
function renderApprovalModal(studentName: string, nim: string, purpose: string): string {
    return `
        <div id="aktif-approval-modal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 animate-fade-in" role="dialog" aria-modal="true" aria-labelledby="aktif-approval-title">
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
                <div class="bg-[#115E59] px-8 py-5 text-white">
                    <h3 id="aktif-approval-title" class="text-lg font-bold">Konfirmasi Persetujuan</h3>
                </div>
                <div class="p-8 space-y-6">
                    ...
                    <div class="space-y-3 bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
                        ...
                        ${renderModalRow('Keperluan', purpose)}
                    </div>
                    <div class="space-y-2">
                        <label for="aktif-nomor-surat" class="text-[11px] font-black text-gray-500 uppercase tracking-widest pl-1">Tambahkan Nomor Surat</label>
                        <input
                ...
                <div class="px-8 pb-8 flex gap-3">
                    <button id="aktif-cancel-approve" type="button" class="flex-1 px-6 py-3.5 border-2 border-gray-100 text-gray-500 font-bold rounded-2xl hover:bg-gray-50 transition-colors text-sm shadow-sm">
                        Batal
                    </button>
                    <button id="aktif-confirm-approve" type="button" class="flex-[1.5] px-6 py-3.5 bg-[#115E59] text-white font-bold rounded-2xl hover:bg-[#0d4a46] transition-all shadow-lg active:scale-[0.98] text-sm">
    ...
    return `
        <div id="${config.id}" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 animate-fade-in" role="dialog" aria-modal="true" aria-labelledby="${config.id}-title">
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden">
                ...
                <div class="p-8 space-y-6">
                    ${config.id === 'aktif-rejection-modal' ? `
...
function bindActionHandlers(applicationId: number, role: string, origin: ReviewerOrigin): void {
    bindModalOpenClose('aktif-approve-btn', 'aktif-approval-modal', 'aktif-cancel-approve');
    bindModalOpenClose('aktif-revise-btn', 'aktif-revision-modal', 'aktif-cancel-revise');
    bindModalOpenClose('aktif-reject-btn', 'aktif-rejection-modal', 'aktif-cancel-reject');

    document.getElementById('aktif-confirm-approve')?.addEventListener('click', async () => {
        const nomorSurat = readTextareaOrInput('aktif-nomor-surat');
        if (!nomorSurat) {
        ...
        await submitAktifAction({
            ...
            buttonId: 'aktif-confirm-approve',
            successFallback: 'Pengajuan berhasil diverifikasi dan diteruskan.',
        });
    });

    document.getElementById('aktif-confirm-revise')?.addEventListener('click', async () => {
        const note = readTextareaOrInput('aktif-revision-note');
        if (!note) {
        ...
        await submitAktifAction({
            ...
            endpoint: 'revise',
            payload: { note },
            buttonId: 'aktif-confirm-revise',
            successFallback: 'Permintaan revisi berhasil dikirim.',
        });
    });

    document.getElementById('aktif-confirm-reject')?.addEventListener('click', async () => {
        const reason = readTextareaOrInput('aktif-rejection-reason');
        if (!reason) {
        ...
        await submitAktifAction({
            ...
            endpoint: 'reject',
            payload: { reason },
            buttonId: 'aktif-confirm-reject',
            successFallback: 'Pengajuan berhasil ditolak.',
        });
    });

    document.addEventListener('keydown', function closeAktifModalsWithEsc(event) {
        if (event.key === 'Escape') {
            closeAllAktifModals();
        }
    }, { once: true });
...
function renderTimeline(app: AktifApplication): string {
    ...
    const steps = [
        ...
        { label: 'Verifikasi Tenaga Pendidik', date: app.tendik_approved_at, done: tendikDoneStatuses.includes(app.status || ''), current: app.status === LETTER_WORKFLOW_STATUS.SUBMITTED },
...
function closeAllAktifModals(): void {
    ['aktif-approval-modal', 'aktif-revision-modal', 'aktif-rejection-modal'].forEach((id) => {
